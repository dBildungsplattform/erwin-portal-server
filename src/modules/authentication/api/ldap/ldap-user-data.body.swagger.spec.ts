...
describe('LdapUserDataBodyParams DTO decorators', () => {
    describe('constructor', () => {
        it('assigns all fields via Object.assign', () => {
            const klasseParams: KlasseLdapImportBodyParams = plainToInstance(KlasseLdapImportBodyParams, {});
            const schuleParams: SchuleLdapImportBodyParams = plainToInstance(SchuleLdapImportBodyParams, {});
            const personParams: PersonLdapImportDataBody = plainToInstance(PersonLdapImportDataBody, {});
            ...
            expect(dto.klasseParams).toBe(klasseParams);
            expect(dto.schuleParams).toBe(schuleParams);
            expect(dto.personParams).toBe(personParams);
    ...
    describe('@Type() + @ValidateNested() + @IsString()', () => {
        it('transforms nested properties into DTO instances (Type decorator)', () => {
            const payload: LdapUserDataBodyParams = {
                klasseParams: {
                    klasseName: 'Testklasse',
                    ldapDn: 'cn=Testklasse,ou=Klassen,dc=example,dc=com',
                },
                schuleParams: {
                    schuleName: 'Testschule',
                    zugehoerigZu: 'Testverband',
                    ldapOu: 'ou=Testschule,dc=example,dc=com',
                },
                personParams: {
                    keycloakUserId: 'user-123',
                    firstName: 'Max',
                    lastName: 'Mustermann',
                    ldapDn: 'uid=max,ou=Users,dc=example,dc=com',
            ...
            expect(dto.klasseParams).toBeInstanceOf(KlasseLdapImportBodyParams);
            expect(dto.schuleParams).toBeInstanceOf(SchuleLdapImportBodyParams);
            expect(dto.personParams).toBeInstanceOf(PersonLdapImportDataBody);
            expect(dto.rolle).toBe(ErwinLdapMappedRollenArt.LEHR);
        });

        it('fails validation when rolle is not a string (IsString decorator)', async () => {
            ...
            // Find the error for "rolle"
            const rolleError: ValidationError | undefined = errors.find((e: ValidationError) => e.property === 'rolle');
            expect(rolleError).toBeDefined();

            // constraints is usually a record like { isString: 'rolle must be a string' }
            const constraints: Record<string, unknown> | undefined = rolleError?.constraints;
    ...
    describe('@ApiProperty() via Swagger document generation', () => {
        ...
        it('exposes DTO properties in OpenAPI schema (ApiProperty decorator)', () => {
            ...
            expect(props).toHaveProperty('klasseParams');
            expect(props).toHaveProperty('schuleParams');
            expect(props).toHaveProperty('personParams');
